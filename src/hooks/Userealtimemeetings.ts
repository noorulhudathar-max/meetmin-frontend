...
// Drop this file into src/hooks/useRealtimeMeetings.ts
...
//     // Realtime subscription — updates the row in state when status changes
...
//             setMeetings(prev =>
//               prev.map(m => m.id === (payload.new as Meeting).id ? { ...m, ...(payload.new as Meeting) } : m)
//             )
